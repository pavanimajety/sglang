...
class FlashInferMLAIndicesUpdaterPrefill:
    ...
    def call_begin_forward(
        ...
        if spec_info is None:
            ...
            if print_cond:
                print(f"DEBUG: [FlashInferMLAIndicesUpdaterPrefill] spec_info is not None", flush=True)
...
class FlashInferMLAMultiStepDraftBackend:
    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch):
        ...
        def call_fn(i, forward_batch):
            ...
            assert isinstance(forward_batch.spec_info, EagleDraftInput)
            if device_index == 0:
                print(f"DEBUG: [FlashInferMLAMultiStepDraftBackend] call_fn step {i}: Cloning kv_indptr and kv_indices", flush=True)
