...
class EAGLEWorker(TpModelWorker):
    ...
    def forward_batch_speculative_generation(
        ...
        if batch.forward_mode.is_extend() or batch.is_extend_in_batch:
            if device_index == 0:
                print(f"DEBUG: [EagleWorker] TARGET EXTEND PATH", flush=True)
                print(f"DEBUG: [EagleWorker- TARGET EXTEND] batch.seq_lens = {batch.seq_lens}", flush=True)
            ...
            if device_index == 0:
                print(f"DEBUG: [EagleWorker- DRAFT EXTEND] Draft extend completed", flush=True)
                print(f"DEBUG: [EagleWorker- DRAFT EXTEND - COMPLETED] next_token_ids: {next_token_ids},  batch.seq_lens = {batch.seq_lens}", flush=True)
